...
class TestAsyncQuery(tb.AsyncQueryTestCase):
    ...
    async def test_async_parse_error_recover_01(self):
        for _ in range(2):
            ...
            for _ in range(10):
                self.assertEqual(await self.client.query("select 1;"), edgedb.Set((1,)))
    ...
    async def test_async_basic_datatypes_01(self):
        for _ in range(10):
            ...
            self.assertEqual(
                await self.client.query('select ["a", "b"]'), edgedb.Set([["a", "b"]])
            )
            ...
            with self.assertRaisesRegex(
                edgedb.InterfaceError,
                r"query_required_single\(\) as it may return " r"more than one element",
            ):
    ...
    async def test_async_basic_datatypes_02(self):
        self.assertEqual(
            await self.client.query(r"""select [b"\x00a", b"b", b'', b'\na']"""),
    ...
    async def test_async_basic_datatypes_03(self):
        for _ in range(10):  # test opportunistic execute
            ...
            self.assertEqual(await self.client.query_json("select (1,)"), "[[1]]")
            ...
            self.assertEqual(
                json.loads(await self.client.query_single_json('select ["a", "b"]')),
                ["a", "b"],
            )
            ...
            self.assertEqual(
                json.loads(await self.client.query_json("SELECT {1, 2}")), [1, 2]
            )
            ...
            with self.assertRaises(edgedb.NoDataError):
                await self.client.query_required_single_json("SELECT <int64>{}")

            self.assertEqual(
                json.loads(await self.client.query_single_json("SELECT <int64>{}")),
                None,
            )
    ...
    async def test_async_args_03(self):
        ...
        with self.assertRaisesRegex(edgedb.InvalidArgumentError, "None is not allowed"):
    ...
    async def test_async_args_04(self):
        ...
        self.assertEqual(
            await self.client.query_single("select <datetime>$0;", aware_datetime),
            aware_datetime,
        )
        ...
        self.assertEqual(
            await self.client.query_single("select <cal::local_date>$0;", date), date
        )

        self.assertEqual(
            await self.client.query_single("select <cal::local_time>$0;", naive_time),
            naive_time,
        )
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single("select <datetime>$0;", naive_datetime)

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, r"a naive time object.*expected"
        ):
            await self.client.query_single("select <cal::local_time>$0;", aware_time)
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single("select <cal::local_datetime>$0;", date)
    ...
    async def _test_async_args_06(self):  # XXX move to edgedb/edgedb
        ...
        self.assertEqual(await self.client.query("select <optional int32>$a", a=1), [1])

        with self.assertRaisesRegex(
            edgedb.InvalidArgumentError, r"argument \$a is required, but received None"
        ):
            self.assertEqual(await self.client.query("select <int32>$a", a=None), [])
    ...
    async def test_async_args_uuid_pack(self):
        obj = await self.client.query_single("select schema::Object {id, name} limit 1")
    ...
    async def test_async_args_bigint_basic(self):
        ...
        val = await self.client.query_single("select <array<bigint>>$arg", arg=testar)
    ...
    async def test_async_args_bigint_pack(self):
        ...
        with self.assertRaisesRegex(edgedb.InvalidArgumentError, "expected an int"):
            await self.client.query("select <bigint>$arg", arg=decimal.Decimal("10.0"))

        with self.assertRaisesRegex(edgedb.InvalidArgumentError, "expected an int"):
            await self.client.query("select <bigint>$arg", arg=decimal.Decimal("10.11"))
        ...
        with self.assertRaisesRegex(edgedb.InvalidArgumentError, "expected an int"):
            ...
            await self.client.query_single("select <bigint>$arg", arg=IntLike())
    ...
    async def test_async_args_decimal(self):
        ...
        with self.assertRaisesRegex(
            ...
            await self.client.query_single("select <decimal>$arg", arg=IntLike())
    ...
    async def test_async_multirange_01(self):
        ...
        if not has_range:
            raise unittest.SkipTest("server has no support for std::multirange")
    ...
    async def test_async_wait_cancel_01(self):
        ...
        client2 = self.make_test_client(database=self.client.dbname).with_retry_options(
                ...
                try:
                    async with TaskGroup() as g:
                        ...
                        with self.assertRaises(asyncio.TimeoutError):
                            ...
                            await asyncio.wait_for(client2.aclose(), timeout=0.5)
    ...
    async def test_enum_argument_01(self):
        ...
        self.assertEqual(await self.client.query_single("SELECT <MyEnum>$0", "A"), A)
    ...
    async def test_json(self):
        self.assertEqual(
            await self.client.query_json('SELECT {"aaa", "bbb"}'), '["aaa", "bbb"]'
        )
    ...
    async def test_async_log_message(self):
        ...
        try:
            await self.client.query("configure system set __internal_restart := true;")
    ...
    async def test_async_banned_transaction(self):
        with self.assertRaisesRegex(
            edgedb.CapabilityError, r"cannot execute transaction control commands"
        ):
