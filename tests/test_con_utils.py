...
class TestConUtils(unittest.TestCase):
    ...
    error_mapping = {
        "credentials_file_not_found": (RuntimeError, "cannot read credentials"),
        ...
        "invalid_dsn_or_instance_name": (ValueError, "invalid DSN or instance name"),
        "invalid_instance_name": (ValueError, "invalid instance name"),
        "invalid_dsn": (ValueError, "invalid DSN"),
        "unix_socket_unsupported": (ValueError, "unix socket paths not supported"),
        ...
        "exclusive_options": (errors.ClientConnectionError, "are mutually exclusive"),
        "env_not_found": (ValueError, 'environment variable ".*" doesn\'t exist'),
        ...
        "invalid_secret_key": (errors.ClientConnectionError, "Invalid secret key"),
    ...
    def run_testcase(self, testcase):
        ...
        with contextlib.ExitStack() as es:
            ...
            es.enter_context(mock.patch("os.stat", lambda _, **__: stat_result))
            ...
            if fs:
                ...
                if homedir:
                    ...
                    es.enter_context(mock.patch("pathlib.Path.home", lambda: homedir))
                if files:
                    ...
                    es.enter_context(
                        mock.patch(
                            "os.path.exists", lambda filepath: str(filepath) in files
                        )
                    )
                    es.enter_context(
                        mock.patch(
                            "os.path.isfile", lambda filepath: str(filepath) in files
                        )
                    )
                    ...
                    es.enter_context(mock.patch("os.path.realpath", lambda f: f))

                    def mocked_open(filepath, *args, **kwargs):
                        if str(filepath) in files:
                            return mock.mock_open(read_data=files.get(str(filepath)))()
                        raise FileNotFoundError(
                            "[Errno 2] No such file or directory: " + f"'{filepath}'"
                        )
            ...
            result = {
                "address": [connect_config.address[0], connect_config.address[1]],
        ...
        for i, testcase in enumerate(testcases):
            with self.subTest(i=i):
                ...
                if wait_until_available:
                    testcase["result"][
                        "waitUntilAvailable"
                    ] = con_utils._validate_wait_until_available(wait_until_available)
    ...
    @mock.patch(
        "edgedb.platform.config_dir", lambda: pathlib.Path("/home/user/.config/edgedb")
    )
