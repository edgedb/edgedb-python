...
class TestSCRAM(unittest.TestCase):
    def test_scram_sha_256_rfc_example(self):
        ...
        SaltedPassword = scram.get_salted_password(
            scram.saslprep(password).encode("utf-8"), base64.b64decode(salt), iterations
        )
        ...
        self.assertEqual(
            scram.B64(ClientProof), "dHzbZapWIk4jUhN+Ute9ytag9zjfMHgsqmmiz7AndVQ="
        )
    ...
    def test_scram_sha_256_verifier(self):
        ...
        self.assertEqual(v, f"SCRAM-SHA-256$4096:{salt}${stored_key}:{server_key}")
